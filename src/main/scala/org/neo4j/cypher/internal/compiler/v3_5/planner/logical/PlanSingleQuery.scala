...
import org.neo4j.cypher.internal.v3_5.logical.plans.{GraphNameSelect, GraphVersionSelect, LogicalPlan}
...
case class PlanSingleQuery(planPart: PartPlanner = planPart,
  ...
  override def apply(in: PlannerQuery, context: LogicalPlanningContext, idGen: IdGen): (LogicalPlan, LogicalPlanningContext) = {
    ...
    if(in.queryGraph.graphVersion.nonEmpty){
      newPlan = GraphVersionSelect(in.queryGraph.graphVersion,finalPlan)(idGen)
      val pQ = finalContext.planningAttributes.solveds.get(finalPlan.id)
      finalContext.planningAttributes.solveds.set(newPlan.id,pQ)
    }
    val graphLookName:Option[String] = in.queryGraph.graphLookName
    if(graphLookName.nonEmpty){
      newPlan = GraphNameSelect(graphLookName.get,finalPlan)(idGen)
      val pQ = finalContext.planningAttributes.solveds.get(finalPlan.id)
      finalContext.planningAttributes.solveds.set(newPlan.id,pQ)
    }
