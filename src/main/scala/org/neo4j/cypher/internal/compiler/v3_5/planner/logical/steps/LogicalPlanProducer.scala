...
case class LogicalPlanProducer(cardinalityModel: CardinalityModel, planningAttributes: PlanningAttributes, idGen: IdGen) extends ListSupport {
  ...
  def planCreate(inner: LogicalPlan, pattern: CreatePattern, context: LogicalPlanningContext): LogicalPlan = {
    ...
    val version:Set[Predicate] = pattern.getVersion
    //val graph = solveds.get(inner.id).amendQueryGraph(_.addPredicates())
    if(version.isEmpty) annotate(plans.Create(inner, pattern.nodes, pattern.relationships), solved, providedOrders.get(inner.id), context)
    else annotate(plans.CreateV(inner, pattern.nodes, pattern.relationships,version), solved, providedOrders.get(inner.id), context)
  }
