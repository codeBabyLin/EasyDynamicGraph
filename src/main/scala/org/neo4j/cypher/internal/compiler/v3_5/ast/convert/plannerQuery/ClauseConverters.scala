...
object ClauseConverters {
  ...
  def addToLogicalPlanInput(acc: PlannerQueryBuilder, clause: Clause): PlannerQueryBuilder = clause match {
    ...
    case c: MatchTime => addMatchTimeToLogicalPlanInput(acc,c)
    case c: GraphLookup => addGraphLookUpToLogicalPlanInput(acc,c)
  ...
  private def addCreateVToLogicalPlanInput(builder: PlannerQueryBuilder, clause: CreateV): PlannerQueryBuilder = {
    ...
    val patten:CreatePattern = CreatePattern(nodes, relationships)
    patten.setVersion(version)
    builder.amendQueryGraph(_.addMutatingPatterns(patten))
    //builder.amendQueryGraph(_.addMutatingPatterns(patten).setGraphVersion(version))
  }
  ...
  private def addMatchTimeToLogicalPlanInput(acc: PlannerQueryBuilder, clause: MatchTime): PlannerQueryBuilder = {
    val patternContent = clause.pattern.destructed

    val selections = asSelections(clause.where)

    val version: Set[Predicate] = clause.time.
      map(_.expression.asPredicates).
      getOrElse(Set.empty)
  ...
  private def addGraphLookUpToLogicalPlanInput(acc: PlannerQueryBuilder, clause: GraphLookup): PlannerQueryBuilder = {
    //val patternContent = clause.pattern.destructed

    //val selections = asSelections(clause.where)
    val graphName = clause.graphName.parts.mkString(".")
    acc.amendQueryGraph{qg => qg.setGraphLookName(graphName)}
