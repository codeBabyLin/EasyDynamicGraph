...
import org.neo4j.cypher.internal.v3_5.logical.plans.{CreateGraphPlan, LogicalPlan}
...
case class CypherPlanner[Context <: PlannerContext](monitors: Monitors,
  ...
  def planPreparedQuery(state: BaseState, context: Context): LogicalPlanState = {
    ...
    val planState = CatalogDDLBuilder.process(state,context)
    val laststate = pipeLine.transform(planState, context)
    CatalogDDLBuilder.process2(planState,laststate)
    //pipeLine.transform(planState, context).withMaybeCataloglPlan(planState.maybeLogicalPlan)
  }
