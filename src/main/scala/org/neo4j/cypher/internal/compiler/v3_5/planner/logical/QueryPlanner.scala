...
case class QueryPlanner(planSingleQuery: SingleQueryPlanner = PlanSingleQuery())
  ...
  def plan(unionQuery: UnionQuery, context: LogicalPlanningContext, solveds: Solveds, cardinalities: Cardinalities, idGen: IdGen): (Option[PeriodicCommit], LogicalPlan, LogicalPlanningContext) =
    unionQuery match {
      case UnionTQuery(queries, distinct, _, periodicCommitHint) =>
      ...
      case ExceptQuery(queries, distinct, _, periodicCommitHint) =>
        ...
        (periodicCommitHint, createProduceResultOperator(newPlan, unionQuery, newContext), newContext)
      case IntersectQuery(queries, distinct, _, periodicCommitHint) =>
        val (plan, newContext) = planQueries(queries, distinct, context, idGen)
        val newPlan = Intersection(plan.rhs.get,plan.lhs.get)(idGen)
        val pQ = newContext.planningAttributes.solveds.get(plan.id)
        newContext.planningAttributes.solveds.set(newPlan.id,pQ)
  ...
  private def createProduceResultOperator(in: LogicalPlan,
    ...
    unionQuery match{
      case x:UnionTQuery =>context.logicalPlanProducer.planProduceResult(in, x.returns, context)
      case x:ExceptQuery =>context.logicalPlanProducer.planProduceResult(in, x.returns, context)
      case x:IntersectQuery =>context.logicalPlanProducer.planProduceResult(in, x.returns, context)
    }
