...
import org.neo4j.cypher.internal.v3_5.expressions.{Expression, Pattern => ASTPattern}
...
trait Clauses extends Parser
  ...
  def MatchTime: Rule1[ast.MatchTime] = rule("MATCHTIME") {
    group((
      keyword("OPTIONAL MATCHTIME") ~ push(true)
        | keyword("MATCHTIME") ~ push(false)
      ) ~~ Pattern ~~ zeroOrMore(Hint, separator = WS) ~~ optional(Where) ~~ optional(TimeRange)) ~~>> (ast.MatchTime(_, _, _, _,_))
  }
...
/* private def TimeRange:Rule1[ast.TimeRange] = rule("TIMERANGE") {
    After | Before | Between ~~>> (ast.TimeRange(_))
  }*/
  private def TimeRange: Rule1[ast.TimeRange] = rule("TIMERANGE") {
    group(keyword("TIMERANGE") ~~ Expression) ~~>> (ast.TimeRange(_))
  }
}
