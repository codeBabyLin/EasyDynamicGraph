...
import cn.DynamicGraph.Version._
...
case class InterpretedPipeBuilder(recurse: LogicalPlan => Pipe,
  ...
  def onOneChildPlan(plan: LogicalPlan, source: Pipe): Pipe = {
    ...
    plan match {
      ...
      case GraphNameSelect(graphName, _) =>
        GraphNameSelectPipe(source, graphName)(id = id)
      case GraphVersionSelect(version, _) =>
        GraphVersionSelectPipe(source, TransFormer.transFormer(version))(id = id)
      case CreateGraphPlan(graphName,_) =>
        CreateGraphPipe(source ,graphName.parts.mkString("."))(id = id)
      ...
      case CreateV(_, nodes, relationships, version) =>
        val ver: FilterVersion = TransFormer.transFormer(version)
        CreatePipeV(
          source,
          nodes.map(n =>
            CreateNodeCommandV(n.idName, n.labels.map(LazyLabel.apply), n.properties.map(buildExpression),ver)
          ).toArray,
          relationships.map(r =>
            CreateRelationshipCommandV(r.idName, r.startNode, LazyType(r.relType.name), r.endNode, r.properties.map(buildExpression),ver)
          ).toArray
        )(id = id)
