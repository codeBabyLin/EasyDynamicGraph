...
sealed class TransactionBoundQueryContext(val transactionalContext: TransactionalContextWrapper,
  ...
  class NodeOperations extends BaseOperations[NodeValue] {
    ...
    private def allWithVersion:Iterator[NodeValue]={
      val iterator = TransactionBoundQueryContext.this.entityAccessor.getGraphDatabase.getDyGraph.AllNodesByFilterVersion()
      new CursorIterator[NodeValue] {
        override protected def fetchNext(): NodeValue = {
          if (iterator.hasNext)
            fromNodeProxy(entityAccessor.newNodeProxy(iterator.next()))
          else null
        }
    ...
    private def allWithName:Iterator[NodeValue] ={
      val iterator = TransactionBoundQueryContext.this.entityAccessor.getGraphDatabase.getDyGraph.AllNodesByGraphName()
      new CursorIterator[NodeValue] {
        override protected def fetchNext(): NodeValue = {
          if (iterator.hasNext)
            fromNodeProxy(entityAccessor.newNodeProxy(iterator.next()))
    ...
    override def all: Iterator[NodeValue] = {
      if(TransactionBoundQueryContext.this.entityAccessor.getGraphDatabase.getDyGraph.isReadByVersion){
        return allWithVersion
      }
      if(TransactionBoundQueryContext.this.entityAccessor.getGraphDatabase.getDyGraph.isReadByName){
        return allWithName
      }
