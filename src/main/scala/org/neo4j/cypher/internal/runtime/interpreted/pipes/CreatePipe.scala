...
import cn.DynamicGraph.Version.{AndFilter, AtFilter, FilterVersion}
...
case class CreatePipeV(src: Pipe, nodes: Array[CreateNodeCommandV],
                       relationships: Array[CreateRelationshipCommandV])
                     (val id: Id = Id.INVALID_ID) extends EntityCreatePipe(src) {
  ...
  override def internalCreateResults(input: Iterator[ExecutionContext], state: QueryState): Iterator[ExecutionContext] = {
    ...
    input.map(row => {
      nodes.foreach { nodeCommand =>
        val (key, node) = createNodeV(row, state, nodeCommand)
        row.set(key, node)
      }

      relationships.foreach { relCommand =>
        val (key, node) = createRelationshipV(row, state, relCommand)
        row.set(key, node)
      }
  ...
  protected def createNodeV(context: ExecutionContext,
                           state: QueryState,
                           data: CreateNodeCommandV): (String, NodeValue) = {
    val labelIds = data.labels.map(_.getOrCreateId(state.query).id).toArray
    //val node:NodeValue = state.query.createNodeV(labelIds,data.version)
    //state.query.setVersion(data.version)
    val node:NodeValue = state.query.createNode(labelIds)
    data.version match{
      case AtFilter(version) => state.query.entityAccessor.getGraphDatabase.getDyGraph.addNode(node.id(),version)
      case AndFilter(AtFilter(v1), AtFilter(v2)) =>
        if(v1>=v2) {
          state.query.entityAccessor.getGraphDatabase.getDyGraph.addNode(node.id(),v2)
          state.query.entityAccessor.getGraphDatabase.getDyGraph.deleteNode(node.id(),v1)
        } else {
          state.query.entityAccessor.getGraphDatabase.getDyGraph.addNode(node.id(),v1)
          state.query.entityAccessor.getGraphDatabase.getDyGraph.deleteNode(node.id(),v2)
        }

        //state.query.entityAccessor.getGraphDatabase.getDyGraph.addNode(node.id(),createVersion)
        //state.query.entityAccessor.getGraphDatabase.getDyGraph.deleteNode(node.id(),deleteVersion)
    }
    data.properties.foreach(setProperties(context, state, node.id(), _, state.query.nodeOps))
    data.idName -> node
  }
   ...
   * Create relationship from command.
   */
  protected def createRelationshipV(context: ExecutionContext,
                                   state: QueryState,
                                   data: CreateRelationshipCommandV): (String, AnyValue) = {
    val start = getNode(context, data.idName, data.startNode, state.lenientCreateRelationship)
    val end = getNode(context, data.idName, data.endNode, state.lenientCreateRelationship)
    ...
    val relationship =
      if (start == null || end == null)
        ...
        val typeId = data.relType.typ(state.query)
        //state.query.setVersion(data.version)
        val relationship: RelationshipValue = state.query.createRelationship(start.id(), end.id(), typeId)
        data.version match{
          case AtFilter(version) => state.query.entityAccessor.getGraphDatabase.getDyGraph.addRelation(relationship.id(),version)
          case AndFilter(AtFilter(v1), AtFilter(v2)) =>
              if(v1>=v2){
                state.query.entityAccessor.getGraphDatabase.getDyGraph.addRelation(relationship.id(),v2)
                state.query.entityAccessor.getGraphDatabase.getDyGraph.deleteRelation(relationship.id(),v1)
              }
              else {
                state.query.entityAccessor.getGraphDatabase.getDyGraph.addRelation(relationship.id(),v1)
                state.query.entityAccessor.getGraphDatabase.getDyGraph.deleteRelation(relationship.id(),v2)
              }
...
case class CreateNodeCommandV(idName: String,
                             labels: Seq[LazyLabel],
                             properties: Option[Expression],version: FilterVersion)

case class CreateRelationshipCommandV(idName: String,
                                     startNode: String,
                                     relType: LazyType,
                                     endNode: String,
                                     properties: Option[Expression],version: FilterVersion)
