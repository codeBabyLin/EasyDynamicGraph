...
public class ShortestPathAStar extends Algorithm<ShortestPathAStar> {
    ...
    public Stream<NameResult> resultStreamWithName() {
        return StreamSupport.stream(shortestPath.spliterator(), false)
                .map(cursor -> new NameResult(dbService.getNodeById(graph.toOriginalNodeId(cursor.value)).getProperty("name").toString(),
    ...
    public static class NameResult {
        ...
        public NameResult(String nodeId, Double cost) {
            this.name = nodeId;
            this.cost = cost;
        }
    }
