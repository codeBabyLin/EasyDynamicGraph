...
abstract class RelationshipLoader {
    ...
    private ContinuousGraph<Long,Long> graph;
    ...
    public void setGraph(ContinuousGraph graph){
        this.graph = graph;
    }
    public ContinuousGraph getGraph(){
        return this.graph;
    }
    ...
    private void visitRelationships(RelationshipSelectionCursor rels, VisitRelationship visit) {
        try (RelationshipSelectionCursor cursor = rels) {
            while (cursor.next()) {
                // version filter
                if(graph!=null && graph.isReadByVersion()){
                    //if(graph.isOkNode(cursor.relationshipReference())
                    long r = cursor.relationshipReference();
                    if(!graph.isOkRelation(r)) continue;
                }
                // version filter
...
final class ReadWithNodeProperties extends RelationshipLoader {
    ...
    @Override
    int load(NodeCursor sourceNode, int localNodeId) {
        this.loader.setGraph(this.getGraph());
