...
import cn.DynamicGraph.Version.AtFilter;
 ...
 * @author mknblch
 */
public class DynamicShortestPathProc {
    ...
    @Procedure("codebaby.shortestPath.astar.stream")
    @Description("CALL codebaby.shortestPath.astar.stream(startNode:Node, endNode:Node, weightProperty:String, graphVersion: long, propertyKeyLat:String," +
    ...
    public Stream<ShortestPathAStar.Result> astarStream(
            ...
            @Name("graphVersion") long graphVersion,
        ...
        if(graphVersion>=1) this.api.getDyGraph().setFilterVersion(new AtFilter(graphVersion));
    ...
    @Procedure("codebaby.shortestPath.astar.streamWithName")
    @Description("CALL codebaby.shortestPath.astar.streamWithName(startNode:Node, endNode:Node, weightProperty:String, graphVersion: long, propertyKeyLat:String," +
            "propertyKeyLon:String, {nodeQuery:'labelName', relationshipQuery:'relationshipName', direction:'BOTH', defaultValue:1.0}) " +
            "YIELD name, cost - yields a stream of {node name, cost} from start to end (inclusive)")
    public Stream<ShortestPathAStar.NameResult> astarStreamWithName(
            @Name("startNode") Node startNode,
            @Name("endNode") Node endNode,
            @Name("propertyName") String propertyName,
            @Name("graphVersion") long graphVersion,
